            ...
            <div id="page-content-wrapper" >
                ...
                <div class="mb-2 mx-2" id="dynamic-body">
                    ...
                    <div align="right" class="mb-2">
                        <form id="filter-bizdeal-onetime-inquiry" method="POST" action="{{'filter-bizdeal-onetime-inquiry'}}">
                        @csrf
                        ...
                        <div class="content-box-email mail-reply-box">
                            ...
                            <div class="py-2 px-2 d-flex justify-content-between">
                                <div class="d-flex w-60">
                                    ...
                                    <p class="mb-0 px-2"><span class=" @if(check_in_my_pin($list))far @else fas @endif fa-flag add-to-pin"></span></p>
                                    <p class="mb-0 ml-2 click-inbox overflow-text-dots-one-line h-1-5-rm @if($list->latestMessageNotMine && $list->latestMessageNotMine->is_read == 0  ) font-weight-bold @endif"
                                ...
                                <div class="d-flex">
                                    <p class="mb-0 click-inbox @if($list->latestMessageNotMine && $list->latestMessageNotMine->is_read == 0  ) font-weight-bold @endif"
                                       data-click-id="{{$list->id}}">
...
$(document).on('click', '.add-pin', function(){
    ...
    $.post("{{route('pin-bizdeal-inquiry-convo-multiple')}}", {
        _token: '{{csrf_token()}}',
        conversation_id: fav,
        json: 'yes'
    }, function (data) {
...
$(document).on('click', '.remove-pin', function(){
    ...
    $.post("{{route('un-pin-bizdeal-inquiry-convo-multiple')}}", {
        _token: '{{csrf_token()}}',
        conversation_id: fav,
        json: 'yes'
    }, function (data) {
...
$(document).on('click', '.star', function(){
    ...
    $.post("{{route('favorite-bizdeal-inquiry-convo-multiple')}}", {
        _token: '{{csrf_token()}}',
        conversation_id: fav,
        json: 'yes'
    }, function (data) {
...
$(document).on('click', '.un-star', function(){
    ...
    $.post("{{route('un-favorite-bizdeal-inquiry-convo-multiple')}}", {
        _token: '{{csrf_token()}}',
        conversation_id: fav,
        json: 'yes'
    }, function (data) {
...
//// add to delete ajax
    ...
    $.post("{{route('delete-bizdeal-inquiry-convo-multiple')}}", {
        _token: '{{csrf_token()}}',
        conversation_id: fav,
        json: 'yes'
    }, function (data) {
...
$(document).on('click', '.un-read', function(){
    ...
    $.post("{{route('unread-bizdeal-inquiry-convo-multiple')}}", {
        _token: '{{csrf_token()}}',
        conversation_id: fav,
        json: 'yes'
    }, function (data) {
...
$(document).on('click', '.read', function(){
    ...
    $.post("{{route('read-bizdeal-inquiry-convo-multiple')}}", {
        _token: '{{csrf_token()}}',
        conversation_id: fav,
        json: 'yes'
    }, function (data) {
...
$(document).on('click', '.click-inbox', function(){
            ...
            $.post("{{route('get-bizdeal-inquiry-messages-inbox')}}", {
                _token: '{{csrf_token()}}',
                conversation_id: id,
                json: 'yes'
            }, function (data) {
                ...
                if (response.feedback == 'encrypt_issue') {
                    ...
                    $.ajax({
                        type: "post",
                        url: "{{route('reply-bizdeal-inquiry-convo')}}",
                        processData: false,
                        contentType: false,
                        data: fd,
                        dataType: "json",
...
$(document).on('click', '.click', function(){
            let id = $(this).data('click-id');
            $.post("{{route('get-bizdeal-inquiry-messages')}}", {
                _token: '{{csrf_token()}}',
                conversation_id: id,
                json: 'yes'
            }, function (data) {
...
/// delete a single conversation ajax
                ...
                $(document).on('click', '.trash-bin', function () {
                    ...
                    $.post("{{route('delete-bizdeal-inquiry-convo')}}", {
                        _token: '{{csrf_token()}}',
...
// add/remove to and from the favorite single conversation
                ...
                $(document).on('click', '.fav', function () {
                    ...
                    let convo_id = $(this).parents('.mail-reply-box').find('.main-convo').attr('data-main-convo');
                    // console.log(convo_id);
                    $.post("{{route('favorite-bizdeal-inquiry-convo')}}", {
                        _token: '{{csrf_token()}}',
                        conversation_id: convo_id,
                        json: 'yes'
                    }, function (data) {
...
/// Add/remove to pin inquiries
                $(document).on('click', '.add-to-pin', function () {
                    ...
                    let convo_id = $(this).parents('.mail-reply-box').find('.main-convo').attr('data-main-convo');
                    // console.log(convo_id);
                    $.post("{{route('pin-bizdeal-inquiry-convo')}}", {
                        _token: '{{csrf_token()}}',
                        conversation_id: convo_id,
                        json: 'yes'
                    }, function (data) {
...
//// to get the listing  according to the filter action button ajax
                ...
                $(document).on('click', '.mails-inbox-icons', function () {
                    ...
                    $.post("{{route('get-filter-inqueries')}}", {
                        _token: '{{csrf_token()}}',
                        filter: filter,
                        from: from,
                        json: 'yes'
...
/// get fresh messages for the sentmail
                $(document).on('click', '#sentMail-tab', function () {
                    ...
                    $("#loader").show();
                    $.post("{{route('get-sent-box-refresh')}}", {
                        _token: '{{csrf_token()}}',
                        user_id: "{{encrypt(\Auth::id())}}",
                        json: 'yes'
                    }, function (data) {
