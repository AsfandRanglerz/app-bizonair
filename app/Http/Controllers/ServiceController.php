...
class ServiceController extends Controller
{
     ...
     public function service_list_by_category($slug)
    {
        ...
        $topbizleadservice = \App\Product::select('products.*', 'products.created_at as creation_date')->where('category_id', $cat->id)->where('product_service_types', 'Service')->with('product_image')->latest()->limit(6)->get();


        $topbuysellservice = \DB::table('buy_sells')->select('buy_sells.*', 'buy_sells.created_at as creation_date')->where('date_expire','>', now())->where('product_service_types','Service')->where('category_id', $cat->id)->whereNull('deleted_at')->latest()->limit(10)->get();
        ...
        $topcompanies = \App\CompanyProfile::limit(2)->latest()->get();
        ...
        $ads = \App\Banner::where('addsdimensions', 'width 1146 * height 161')->where('description','1st row')->where('heading','Textile Services')->where('status', 1)->limit(1)->get();
    ...
    public function serviceDetail($category, $subcategory ,$slug)
    {
        ...
        if($product){
            ...
            $osdts = \App\Product::select('products.*', 'products.created_at as creation_date')->where('product_service_types', 'Service')->where('company_id', $product->company_id)->with('product_image')->latest()->paginate(15);

            $ssdos = \App\Product::select('products.*', 'products.created_at as creation_date')->where('product_service_types', 'Service')->where('subcategory_id',$product->subcategory_id)->where('company_id', '!=', $product->company_id)->with('product_image')->latest()->paginate(15);

            $sdfc = \App\Product::select('products.*', 'products.created_at as creation_date')->where('product_service_types', 'Service')->where('subcategory_id',$product->subcategory_id)->where('origin', $product->origin)->with('product_image')->latest()->paginate(15);
            $cats = \App\Category::where('type', 'Services')->get();
            $ads = \App\Banner::where('addsdimensions', 'width 300 * height 477')->where('description','1st row right sidebar')->where('heading','Textile Services Detail')->where('status', 1)->limit(1)->get();
    ...
    public function service_list_by_subcategory_regular_service($category, $subcategory)
    {
        ...
        $products = \App\Product::select('products.*', 'products.created_at as creation_date')->where('product_service_types', 'Service')->where('subcategory_id', $subcatid)->with('product_image')->latest()->get();
    ...
    public function view_all_regular_service($slug)
    {
        ...
        $toptensellproduct = \App\Product::select('products.*', 'products.created_at as creation_date')->where('category_id', $cat->id)->where('product_service_types', 'Service')->with('product_image')->latest()->limit(6)->get();
        $top_6 = array_unique(\Arr::pluck($toptensellproduct,'id'));
        $topsellproduct = \App\Product::select('products.*', 'products.created_at as creation_date')->where('category_id', $cat->id)->whereNotIn('id',$top_6)->where('product_service_types', 'Service')->with('product_image')->get();
    ...
    public function view_all_service_deals($slug)
    {
        ...
        $buyselltopten_selling = \DB::table('buy_sells')->select('buy_sells.*', 'buy_sells.created_at as creation_date')->where('date_expire','>', now())->where('product_service_types','Service')->where('category_id', $cat->id)->whereNull('deleted_at')->latest()->limit(6)->get();
