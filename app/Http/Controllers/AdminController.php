...
class AdminController extends VoyagerBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        return view('vendor.voyager.userss.browse', compact(
            'actions',
            'dataType',
            'dataTypeContent',
            'isModelTranslatable',
    ...
    public function block_subadmin($id){
        $subadmin = \App\User::find($id);
        if($subadmin){
            $subadmin->is_blocked = 1;
            if($subadmin->save()){
                if (auth()->user()->can('browse', 'users')) {
                    $redirect = redirect()->route("voyager.users.index");
                } else {
                    $redirect = redirect()->back();
                }
    
                return $redirect->with([
                    'message'    => 'Successfully Blocked Sub Admin',
                    'alert-type' => 'success',
                ]);
            }
        }
    ...
    public function unblock_subadmin($id){
        $subadmin = \App\User::find($id);
        if($subadmin){
            $subadmin->is_blocked = 0;
            if($subadmin->save()){
                if (auth()->user()->can('browse', 'users')) {
                    $redirect = redirect()->route("voyager.users.index");
                } else {
                ...
                return $redirect->with([
                    'message'    => 'Successfully Unblocked Sub Admin',
                    'alert-type' => 'success',
                ]);
            }
        }
